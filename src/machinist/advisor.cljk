(ns machinist.advisor
  "Machinist Advisor — proposing a metal-working machine-shop scheduling/
  logistics coordination operation (log a work record, schedule a crew
  ...
  proposes — `machinist.governor` independently gates every proposal
  and always escalates safety concerns and above-threshold supply
  orders. The advisor never proposes to directly finalize a
  machine-setup or machining-execution decision (e.g. a specific
  lathe, mill or press setup/run) or to override a shop safety
  officer's judgment — those stay permanently out of this actor's
  scope. Modeled on cloud-itonami-isco-7222's advisor for the
  closely-related machine-shop safety-domain shape.
...
(defn- rationale-for [op worker-id workshop-id hazard-type]
  (case op
    ...
    (str "scheduled crew operation for machine tool changeover at workshop " workshop-id)
...
(def ^:private system-prompt
  "You are a metal-working machine-shop scheduling/logistics
   ...
   propose to directly finalize a machine-setup or
   machining-execution decision (e.g. deciding to proceed with a
   specific lathe, mill or press setup/run), or to override a shop
   ...
   sets up or operates machine tools or authorizes machining work
