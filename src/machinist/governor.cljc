(ns machinist.governor
  "MachinistGovernor — the independent safety/scope layer gating every
  workshop scheduling/logistics proposal an advisor may make for a
  metal-working machine-shop crew. The governor never dispatches
  hardware itself, never sets up or operates CNC or manual machine
  tools (lathes, mills, presses) on the shop floor, and never
  finalizes a machine-setup or machining-execution decision (e.g.
  deciding to proceed with a specific lathe, mill or press setup/run)
  or overrides a shop safety officer's judgment — those are
  ...
  actor coordinates WORKSHOP SCHEDULING/LOGISTICS ONLY — it never sets
  up or operates machine tools itself). Modeled on
  cloud-itonami-isco-7222's toolmaker.governor for the closely-related
  ...
  HARD invariants (:hard? true, ALWAYS :hold, never overridable):
    ...
    3. no-actuation           — proposal :effect must be :propose (the
                                ...
                                never sets up or operates a machine
                                tool itself; it only gates what the
    ...
    5. scope-excluded action  — any proposal to directly finalize a
                                machine-setup or machining-execution
                                decision (e.g. deciding to proceed
                                with a specific lathe, mill or press
                                setup/run), or to override a shop
                                ...
                                \"proceed with the machine setup\" /
                                \"authorize the machining operation\" /
                                \"override the shop safety officer's
                                judgment\", never as bare nouns like
                                \"machine\" or \"safety\", so the check
                                can never self-trip on the advisor's
                                own routine rationale text, e.g.
                                \"logged work record for worker …\" or
                                \"scheduled crew operation for machine
                                tool changeover …\" or \"…routed for
...
(def ^:private scope-excluded-ops
  #{:finalize-machine-setup-decision :authorize-machining-operation
    :proceed-with-machine-setup :override-safety-officer-judgment
    :override-shop-safety-officer-judgment})

;; Full finalization/execution ACTION phrases only — never bare nouns
;; ("machine", "lathe", "mill", "press", "safety", "shop safety
...
;; nouns, e.g. "machine tool changeover" / "shop safety officer
;; review"). See
;; `governor-test/default-mock-advisor-proposals-never-self-trip-on-scope-exclusion`.
(def ^:private scope-excluded-phrases
  ["proceed with the machine setup" "proceed with the machining operation"
   "finalize the machine setup decision" "authorize the machining operation"
   "authorize the machine setup"
  ...
  (let [{:keys [op rationale]} proposal]
    (cond-> []
      ...
      (conj {:rule :scope-excluded-action
             :detail "機械設定・機械加工の実行判断（旋盤/フライス/プレス等の setup/run）の確定・shop safety officer の判断の上書きは、この actor の権限外 — 常に永続ブロック"}))))

(defn check
  "Assess a proposal against `request`/`context`/`proposal` and a
  `store` implementing `machinist.store/Store`. Pure — never mutates
