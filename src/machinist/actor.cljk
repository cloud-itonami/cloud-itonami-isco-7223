(ns machinist.actor
  "MachinistActor — the ISCO-08 7223 metal working machine tool
  ...
  toolmaker.actor for the closely-related machine-shop safety-domain
  shape.
  ...
  The unconditional invariant: the Machinist Advisor can never
  directly commit an operating record the MachinistGovernor refuses,
  and can never make this actor finalize a machine-setup or
  machining-execution decision (e.g. a specific lathe, mill or press
  setup/run) or override a shop safety officer's judgment — every
  ...
  sets up or operates CNC or manual machine tools or authorizes
  machining work itself."
...
(defn build-graph
  "Build a compiled MachinistActor graph. `store` implements
  `machinist.store/Store`. `advisor` implements
  `machinist.advisor/Advisor` (defaults to `mock-advisor`).
