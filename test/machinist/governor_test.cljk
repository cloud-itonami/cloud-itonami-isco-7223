(ns machinist.governor-test
  (:require [clojure.test :refer [deftest is testing]]
            [machinist.store :as store]
            [machinist.advisor :as advisor]
            [machinist.governor :as governor]))
...
(deftest hard-on-scope-excluded-op-finalize-machine-setup-decision
  (testing "finalizing a machine-setup decision is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :finalize-machine-setup-decision) :confidence 0.99) st)]
      (is (:hard? v))
...
(deftest hard-on-scope-excluded-op-proceed-with-machine-setup
  (testing "proceeding with a machine setup is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :proceed-with-machine-setup) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest hard-on-scope-excluded-rationale-even-with-allowlisted-op
  (testing "defense-in-depth: a rationale that itself attempts to proceed with the machine setup is blocked even if the op looks routine"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :schedule-crew-operation)
                                           :rationale "recommend we proceed with the machine setup now")
                             st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest default-mock-advisor-proposals-never-self-trip-on-scope-exclusion
  (testing "the governor's scope-exclusion term list must never match the mock advisor's own default rationale text for any allowlisted op — CLAUDE.md's known self-tripping bug pattern (rationale legitimately contains bare nouns like 'machine'/'shop safety officer', but never the full finalization-action phrases)"
    (let [st (fresh-store)
      ...
      (doseq [o ops]
        (let [request {:worker-id "worker-1" :op o :workshop-id "WS-1"
                        :stake :low :task "routine machine-shop task"
